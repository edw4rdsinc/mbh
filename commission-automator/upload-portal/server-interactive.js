...
async function completeProcessing(sessionId) {
  ...
  try {
    ...
    console.log(`📋 Wrote ${skippedForReviewCount} entries to needs_review.csv (user skipped items only)`);
    ...
    if (existsSync(csvPath) && existsSync(needsReviewPath)) {
      ...
      console.log(`  - needs_review.csv: ${skippedForReviewCount} entries`);
    } else {
    ...
    console.log(`📧 Starting report generation for ${session.month}...`);
    await runPythonScript('generate_report.py', ['--month', session.month]);
    console.log(`✅ Report generation completed! Sending completion message to client...`);
    ...
    console.log(`📊 Completion stats:`, JSON.stringify(completionStats, null, 2));
...
async function sendStatus(sessionId, message, extra = {}) {
  ...
  if (!session || !session.ws) {
    console.error(`❌ Cannot send status - session ${sessionId} not found or no WebSocket`);
    return;
  }
  ...
  try {
    ...
    console.log(`📤 Sending status to ${sessionId}: ${message} (phase: ${extra.phase || 'none'})`);
    await session.ws.send(JSON.stringify(payload));
